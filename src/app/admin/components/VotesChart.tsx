...
export default function VotesChart() {
    ...
    useEffect(() => {
        const fetchEarliest = async () => {
            ...
            if (data && data.length > 0) {
                setEarliestDate(data[0].voted_at.split('T')[0]);
            }
        };
        fetchEarliest();
    }, []);
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            let inPeriodCount = 0;
            allRows.forEach((row) => {
                if (!row.voted_at) return;
                const dateString = new Date(row.voted_at).toISOString().split('T')[0];
    ...
    if (loading) return (
        <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-100 flex flex-col h-[460px]">
            <h3 className="text-lg font-bold text-gray-900">Votes</h3>
            ...
            <div className="flex flex-wrap justify-between items-center mb-6 gap-3">
                ...
                <div className="flex items-center gap-3">
                    ...
                    <span className="text-xs text-gray-400">
                        {debugInfo.totalInPeriod} votes in period ({debugInfo.fetched} total scanned)
                    </span>
                </div>
            </div>
            <div className="h-[350px] w-full">
                <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={data} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
                                ...
                                if (active && payload && payload.length) {
                                    ...
                                    return (
                                        <div className="bg-white p-3 border border-gray-100 shadow-lg rounded-xl min-w-[120px]">
                                            ...
                                            <p className="text-sm font-bold text-amber-600">{payload[0].value} votes</p>
                                        </div>
                                    );
                                }
                                return null;
                        ...
                        <Bar dataKey="count" name="Votes" fill="#F59E0B" radius={[4, 4, 0, 0]} />
                    </BarChart>
                </ResponsiveContainer>
            </div>
        </div>
